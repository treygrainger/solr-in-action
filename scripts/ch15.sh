...
curl "http://localhost:8983/solr/salestax/select?q=*%3A*&fq%3D%7B!frange%20l%3D10%20u%3D15%7Dproduct(basePrice%2C%20sum(1%2C%24userSalesTax))&userSalesTax=0.07"
echo -e "\n"
echo -e "pg 507"
echo -e "\n"
curl "http://localhost:8983/solr/salestax/select?q=*:*&userSalesTax=0.07&fl=id,basePrice,product(basePrice,%20sum(1,%20%24userSalesTax))"
java -jar $SOLR_IN_ACTION/solr-in-action.jar listing 15.1
echo -e "\n"
echo -e "pg 508"
echo -e "\n"
curl "http://localhost:8983/solr/salestax/select?q=*:*&userSalesTax=0.07&sort=product(basePrice,%20sum(1,%20%24userSalesTax))%20asc,%20score%20desc"
curl "http://localhost:8983/solr/salestax/select?q=_query_:%22%7B!func%7Drecip(ord(date),1,100,100)%22&userSalesTax=0.07&totalPriceFunc=product(basePrice,sum(1,%24userSalesTax))&fq=%7B!frange%20l=10%20u=15%20v=%24totalPriceFunc%7D&fl=*,totalPrice:%24totalPriceFunc&sort=%24totalPriceFunc%20asc,score%20desc"
echo -e "\n"
echo -e "pg 520"
echo -e "\n"
java -Durl=http://localhost:8983/solr/customfunction/update -jar post.jar ch15/documents/customfunction.xml
curl "http://localhost:8983/solr/customfunction/select?q=*:*&fl=introduction:concat(concat(field1,field2,%20%22,%20%22),%22!%22)"
echo -e "\n"
echo -e "pg 522"
echo -e "\n"
cd $SOLR_IN_ACTION/example-docs/
...
echo -e "pg 523"
echo -e "\n"
curl "http://localhost:8983/solr/geospatial/select?q=*:*&fq=%7B!geofilt%20sfield=location%20pt=37.775,-122.419%20d=20%7D"
echo -e "\n"
echo -e "pg 524"
echo -e "\n"
curl "http://localhost:8983/solr/geospatial/select?q=*:*&fq=%7B!bbox%20sfield=location%20pt=37.775,-122.419%20d=20%7D"
echo -e "\n"
echo -e "pg 525"
echo -e "\n"
...
echo -e "pg 527"
echo -e "\n"
curl "http://localhost:8983/solr/geospatial/select?q=*:*&fq=%7B!geofilt%20sfield=location%20pt=37.775,-122.419%20d=20%7D&fl=*,distance:geodist(location,%2037.775,-122.419)&sort=geodist(location,%2037.775,-122.419)%20asc,%20score%20desc"
curl "http://localhost:8983/solr/geospatial/select?q=*:*&fq=%7B!geofilt%7D&fl=*,distance:geodist()&sort=geodist()%20asc,score%20desc&sfield=location&pt=37.775,-122.419&d=20"
echo -e "\n"
echo -e "pg 532"
echo -e "\n"
for ID in `ps waux | grep java | grep [s]tart.jar | awk '{print $2}' | sort -r`
...
#next line has an issue...
curl "http://localhost:8983/solr/geospatial/select?q=*:*&fl=id,location_rpt,city&fq=location_rpt:%22IsWithin(POLYGON((-85.4997%2034.7442,-84.9723%2030.6134,-81.2809%2030.5255,-80.9294%2032.0196,-83.3024%2034.8321,-85.4997%2034.7442)))%20distErrPct=0%22"
echo -e "\n"
echo -e "pg 535"
echo -e "\n"
curl "http://localhost:8983/solr/geospatial/select?sort=score%20asc&q=%7B!geofilt%20pt=45.15,-93.85%20sfield=location%20d=5%20score=distance%7D"
curl "http://localhost:8983/solr/geospatial/select?sort=score%20asc&q=%7B!geofilt%20pt=37.775,-122.419%20sfield=location_rpt%20d=5%20score=distance%20filter=false%7D"
curl "http://localhost:8983/solr/geospatial/select?sort=%24distance%20asc&fl=id,distance:%24distance&q=*:*&distance=query(%24distFilter)&distFilter=%7B!geofilt%20pt=37.775,-122.419%20sfield=location_rpt%20d=5%20score=distance%20filter=true%7D"
echo -e "\n"
echo -e "pg 536"
echo -e "\n"
cd $SOLR_IN_ACTION/example-docs/
