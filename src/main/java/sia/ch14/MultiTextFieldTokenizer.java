...
public class MultiTextFieldTokenizer extends Tokenizer {
	...
	protected MultiTextFieldTokenizer(IndexSchema indexSchema, Reader input, String fieldName, MultiTextFieldSettings settings) {
		super(input);
	...
	private LinkedHashMap<String, Analyzer> getNamedAnalyzers(MultiTextFieldInput multiTextInput){
		...
		for (int i = 0; i < this.multiTextInput.Keys.size(); i++){
			...
			if (fieldType != null){
				...
				if (!this.settings.ignoreMissingMappings){
					throw new SolrException(SolrException.ErrorCode.BAD_REQUEST, "Invalid FieldMapping requested: '" + this.multiTextInput.Keys.get(i) + "'");
				}
			}
		}
		...
		if (namedAnalyzers.size() < 1){
			if (this.settings.defaultFieldTypeName != null && this.settings.defaultFieldTypeName.length() > 0) {
				if (this.settings.analyzerMode == AnalyzerModes.query){
					namedAnalyzers.put("", this.indexSchema.getFieldTypeByName(this.settings.defaultFieldTypeName).getQueryAnalyzer());
				}
				else if (this.settings.analyzerMode == AnalyzerModes.multiTerm){
					namedAnalyzers.put("", 
							((TextField)this.indexSchema.getFieldTypeByName(this.settings.defaultFieldTypeName)).getMultiTermAnalyzer());
				}
				else{
					namedAnalyzers.put("", this.indexSchema.getFieldTypeByName(this.settings.defaultFieldTypeName).getAnalyzer());
				}
			}
		...
		if (namedAnalyzers.size() == 0){
			throw new SolrException(SolrException.ErrorCode.BAD_REQUEST, "No FieldMapping was Requested, and no DefaultField"
					+ " is defined for MultiTextField '" + this.fieldName + "'. A MultiTextField must have one or more " +
	...
	private SortedMap<Integer, LinkedList<Token>> createPositionsToTokensMap(LinkedHashMap<String, Analyzer> namedAnalyzers, String text) throws IOException{
		...
		// position to preserve cross-language phrase searching and allow duplicates to be removed
        SortedMap<Integer, LinkedList<Token>> tokenHash = new TreeMap<Integer, LinkedList<Token>>();

        //add the tokenstream for each sub-field type into the position to token mapping
        for (Map.Entry<String, Analyzer> namedAnalyzer : this.namedAnalyzers.entrySet()) {
            ...
            addTokenStreamForFieldType(tokenHash, namedAnalyzer.getValue().tokenStream(subFieldName, new StringReader(text)));
        }
	 ...
	 * @throws IOException 
     */
	private void addTokenStreamForFieldType(SortedMap<Integer, LinkedList<Token>> tokenHash, TokenStream tokenStream) throws IOException {
